...
{
	tele_type = "Reflector Telescope";
}

//Look will return true only if within the rise/fall time for that planet.
//If its not visible at that time, will return false. Well get the data for this
//by looking at future star maps based around the assesment day
bool Newtonian_Reflector::look(std::string planet, std::string time)
{
		//Making system
...
//getter
std::string Newtonian_Reflector::getType()
{
	return tele_type;
}
