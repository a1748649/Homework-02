...
//Constructor
Dwarf_Planet::Dwarf_Planet(std::string planet_name, std::string a_rise_time, std::string a_fall_time, std::string a_colour)
{
	name = planet_name;
	type = "Dwarf Planet";
	colour = a_colour;
...
//Behaviour
void Dwarf_Planet::printDetails(string planet_name, string tele_type, std::string time)
{
	//Details
	...
	for (int i = 0; i < number_of_planets; ++i)
	{
		...
		{
			//cout << "name being passed into is "<<planet_name<<", planet name being checked right now is "<<arrplanet_names[i]<<endl;
		}
	}
}
...
{
	//cout << "Destroying Dwarf Planet from child class"<<endl;
}